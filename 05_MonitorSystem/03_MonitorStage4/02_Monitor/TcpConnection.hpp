...
public:
    ...
    // void sendAVPacket(AVPacket* pkt, AVStream* stream);     // 发送 AVPacketTLV
    ...
    void setUserName(string ip, const string & name) { m_username[ip] = name; } 
    string getUserName(string ip) { return m_username[ip]; } 

    void setUserSalt(string ip, const string & salt) { m_userSalt[ip] = salt; } 
    string getUserSalt(string ip) { return m_userSalt[ip]; } 
...
private:
    ...
    std::unordered_map<string, string> m_username;      // 存放已登陆的用户名称
    std::unordered_map<string, string> m_userSalt;      // 存放首次登陆用户的盐值
};
